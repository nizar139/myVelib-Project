/**
 * MechanicalOnlyRide is a subclass of RidesPlaning.
 * It represents a planning strategy where only stations with mechanical bicycles are considered.
 */
...
public class MechanicalOnlyRide extends RidesPlaning {
    /**
     * This method generates a plan for the ride considering only mechanical bikes.
     * It finds the nearest DockingStation with at least one mechanical bike for starting,
    ...
    public String getYourPlan(ArrayList<DockingStation> stations, ArrayList<Bicycle> bikes, double[] startingGPS, double[] endingGPS) {
        ...
        for (DockingStation station : stations) {
            if (station.getStatus() == StationStatus.ONSERVICE) {
                if (station.getNumberOfMechanicalBikes() > 0) {
        ...
        return "Nearest starting station with at least one Mechanical Bike: " + nearestStartingStation + ", Nearest ending station: " + nearestEndingStation;
    }
}
